...
def valid_mark(mark):
    ...
    if hasattr(mark, 'll'): mark = mark.ll
    try: x,y = float(mark[0]), float(mark[1])
    except: return False
    return True
...
class Leaflet(JS):
    ...
    def __init__(self, centroid=centroid_bc, zoom=default_zoom, height=default_height, width=default_width,
                        control=None ):
    ...
    def head(self):
        """TODO: only return what we need for this map"""
        return leaflet_head
    ...
    def set_tiles(self, tiles):
        t=[]
        for tile in tiles:
            if tile in available_tilesets.keys(): t.append(tile)
    ...
    def tilesets(self):
        ...
        return '\n'.join(
            ['var %s = L.tileLayer("%s", {attribution: \'%s\' });' % (
                var_for(provider),
                tile_for(provider)[0],
                tile_for(provider)[1]) for provider in self.tiles
             ])
...
class HeatLayer(LayerGroup):
    ...
    head = """    <script src="/static/dz/heatmap.js/heatmap.min.js"></script>
