# coding=utf8

from model import *
from zoom.response import *

form1 = Form(
        TextField('Name', addon='$', hint='test hint'),
        EmailField('Email', hint='test hint'),
        PhoneField('Phone', hint='phone number'),
        PostalCodeField('Postal Code', hint='test hint'),
        ImagesField('Photos', url='/fieldsdev/', hint='click or drag and drop to upload'),
        TwitterField('Twitter', hint='test hint'),
        URLField('Web', hint='test hint'),
        PasswordField('Password', hint='test hint'),
        NumberField('Number of Things', hint='test hint'),
        NumberField('Area', units='ft<sup>2</sup>', hint='test hint'),
        IntegerField('Count', hint='test hint'),
        FloatField('Size', units='meters', hint='test hint'),
        DecimalField('Decimal', hint='test hint'),
        MoneyField('Amount', hint='test hint'),
        MoneyField('Foreign Amount', symbol='£', hint='test hint'),
        DateField('Date', hint='test hint'),
        BirthdateField('Birth Date', hint='test hint'),
        CheckboxesField('Select', values=['One','Two','Three'], hint='test hint'),
        CheckboxField('Publish', hint='test hint'),
        RadioField('Radio', values=['One','Two','Three'], hint='test hint'),
        PulldownField('Pulldown', options=['One','Two','Three'], hint='test hint'),
        MultiselectField('Multiselect', options=['One','Two','Three'], hint='test hint'),
        ChosenMultiselectField('Chosen', options=['One','Two','Three'], hint='test hint'),
        ButtonField('Okay!', hint='test hint'),
        MemoField('Memo', hint='test hint'),
        MarkdownField('Markdown', hint='test hint'),
        EditField('Editor', hint='test hint'),
        ButtonField('Save', cancel='/{}/forms'.format(system.app.name))
        )

sample = dict(
        name = 'Joey',
        email = 'joey@dsilabs.ca',
        postal_code = 'V8X 2P2',
        phone = '2341231234',
        twitter = 'dsilabs',
        web = 'https://www.dsilabs.ca',
        password = 'mypass',
        number_of_things = 123,
        count = 456,
        size = 1.2345,
        decimal = Decimal('1234132.234'),
        amount = Decimal('12341232.32432'),
        foreign_amount = Decimal('121232.432'),
        date = datetime.date(2015,4,2),
        birth_date = datetime.date(2073,2,18),
        select = 'One',
        publish = False,
        radio = 'One',
        pulldown = 'Two',
        multiselect = ['One','Three'],
        chosen = ['One', 'Two'],
        memo = 'memo content',
        markdown = 'Heading\n====\nmarkdown content',
        editor = 'editor content',
        )

def menu_item(name):
    return (name, '/{}/forms/{}'.format(system.app.name, name.lower()))

actions = [
    menu_item('Show'),
    menu_item('Edit'),
    menu_item('New'),
    ]

js = """
"""

def image_response(name, data):
    _, ext = os.path.splitext(name.lower())
    if ext == '.png':
        return PNGResponse(data)
    elif ext == '.jpg':
        return JPGResponse(data)
    elif ext == '.gif':
        return GIFResponse(data)

class MyView(View):

    def index(self, q=""):
        form1.initialize(sample)
        content = form1.show()
        return page(content, actions=actions, title='Overview')

    def show(self, key=None):
        form1.initialize(sample)
        content = form1.show()
        return page(content, actions=actions, title='Show Mode')

    def edit(self, key=None):
        path = os.path.join(system.site.data_path, 'buckets')
        bucket = Bucket(path)
        form1.update(sample)
        content = form1.edit()
        return page(content, actions=actions, title='Edit Mode')

    def new(self):
        form1.initialize({})
        content = form1.edit()
        return page(content, actions=actions, title='Edit Mode')

    def list_images(self, key=None, field=None):
        """return list of images for an ImagesField for this record"""
        path = os.path.join(system.site.data_path, 'buckets')
        bucket = Bucket(path)
        t = [dict(
            name='house{}.jpg'.format(n),
            size=len(bucket.get(item_id)),
            item_id=item_id,
            url=url_for('get_image', item_id=item_id),
            ) for n,item_id in enumerate(bucket.keys()) if not key or key in item_id]
        return json.dumps(t)

    def get_image(self, item_id=None):
        """return an image response for an image"""
        path = os.path.join(system.site.data_path, 'buckets')
        bucket = Bucket(path)
        return image_response('house.png', bucket.get(item_id))


class MyController(Controller):

    def save_button(self, *a, **k):
        if form1.validate(k):
            return page('saved', title='Result')

    def add_image(self, *a, **k):
        """add a dropzone image"""
        # k contains FieldStorage object containing filename and data
        path = os.path.join(system.site.data_path, 'buckets')
        bucket = Bucket(path)
        f = k.get('file')
        name = f.filename
        data = f.file.read()
        item_id = bucket.put(data)
        return item_id

    def remove_image(self, *a, **k):
        """remove a dropzone image"""
        # k contains item_id and filename for file to be removed
        path = os.path.join(system.site.data_path, 'buckets')
        bucket = Bucket(path)
        items = bucket.keys()
        item_id = k.get('id', None)
        if item_id in bucket.keys():
            bucket.delete(item_id)
            return 'ok'
        return 'empty'



view = MyView()
controller = MyController()


